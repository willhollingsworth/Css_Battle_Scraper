...
class Main_downloader():
    ...
    def download_all_main_file(self):
        for name,url in self.main_urls.items():    
            data = self.download_data(url)
            if self.check_type(name,data) == 'html':
                self.download_main_html(name,data)
            else :
                self.download_main_json(name,data)
    ...
    def download_main_json(self,name,data) -> None:
        filename = 'main.json'
        path = f'./cache/json/{name}/'
        full_path = path + filename
        self.save_json(data,full_path)

    def download_main_html(self,name,data) -> None:
        filename = 'main.html'
    ...
    def create_all_required_folders(self):
        ...
        for folder in self.required_folders:
            ...
            if folder == './cache' : # dont create subfolder on root folder
                continue      
            for name in self.main_urls.keys():
...
class Main_parser():
    ...
    def get_single_names_automedon(self):
        html_main = './cache/html/automedon/main.html'
...
if __name__ == '__main__':
    ...
    downloader.download_all_main_file()
